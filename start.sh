...
DETECT_SCRIPT="$SCRIPT_DIR/detect_xdp_deps.sh"
...
detect_xdp_dependencies() {
    echo -e "${BLUE}Detecting XDP/BPF dependencies...${NC}"
    echo ""
    
    if [ ! -f "$DETECT_SCRIPT" ]; then
        echo -e "${RED}ERROR: detect_xdp_deps.sh not found${NC}"
        exit 1
    fi
    ...
    if ! "$DETECT_SCRIPT"; then
        echo -e "${RED}ERROR: XDP dependency detection failed${NC}"
        echo ""
